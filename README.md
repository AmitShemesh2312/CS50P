...
Design Choices
I chose to render movie posters as ASCII art rather than opening them in an image viewer because the program is meant to feel like a CLI tool, not a wrapper around a GUI. Having an image window pop up would have broken the flow. I apply a contrast enhancement of 1.5x with pillow before passing the image to ascii-magic because, without it, darker posters tended to render as a muddy block of similar characters. The contrast bump makes the shapes much more readable in ASCII.
I split the saved-movies feature into two separate lists — Watchlist and Hearts — instead of one flat "favorites" list, because the two have different meanings to me. The Watchlist is aspirational (movies I want to see), and the Hearts list is retrospective (movies I have already seen and loved). I wanted the interface to reflect that distinction rather than collapse it.
For storage I considered using JSON, but CSV felt more honest to the "list" mental model and is easy to inspect or even edit by hand if needed. I used csv.DictReader and csv.DictWriter so that adding more columns later (release year, rating at the time of save, etc.) would be a small change.
For the command-line interface I went with argparse and a single -m flag plus a positional movie_words argument. I considered subcommands (search / list) but the flat interface felt friendlier for casual use, and the program does not have enough modes to justify the extra structure.
Setup
